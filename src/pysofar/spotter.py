...
from pysofar.wavespectra.spectrum1D import WaveSpectrum1D, WaveSpectrum1DInput
from pysofar.wavefleet_exceptions import ExceptionNoFrequencyData
import typing
    ...
    def spectra(
        ...
        if not json_data['frequencyData']:
            raise ExceptionNoFrequencyData(
                f'Spotter {self.id} has no spectral data for the requested time range')
